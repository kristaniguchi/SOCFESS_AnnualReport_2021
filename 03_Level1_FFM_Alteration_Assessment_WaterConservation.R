#01_Flow Ecology Level 1 - Alteration Assessment based on water conservation scenario deviation from reference
  #This code evaluates alteration of functional flow metrics (FFM) based on comparison of watercon FFM to reference ranges
...
for (i in 1:length(sites)){
  ...
  for(j in 1:length(ffm.list)){
    ...
    #if watercon or ref has < 5 metric values, then skip alteration assessment and add to alteration notes
    if(FFM.percentiles.sub.ffm$metric_count[FFM.percentiles.sub.ffm$dataset == "watercon"] < 5 |
      ...
      out.row <- c("", sites[i], "", ffm.list[j], "Not enough values", "", "", unique(FFM.percentiles.sub.ffm$region), unique(FFM.percentiles.sub.ffm$scenario), "watercon", "Less than 10 metric values, cannot do assessment")
      #save in output df
      alteration.df.overall <- rbind(alteration.df.overall, out.row)
    }else{
      #else, if >=5 values, then run alteration assessment
      
      #model results and median for ffm i
      model.watercon.ffms.i <- FFM.annual.sub.ffm$watercon_value_final[!is.na(FFM.annual.sub.ffm$watercon_value_final)]
      model.watercon.i.med <- FFM.percentiles.sub.ffm$p50[FFM.percentiles.sub.ffm$dataset == "watercon"]
      ...
      #if NA values for watercon or reference ffms, then all are NA
      if(is.na(model.watercon.i.med) |  is.na(model.ref.i.90)){
        alteration.status[j] <- NA
        alteration.direction[j] <- NA
      }else{
        #else, if median falls outside of 10-90, likely altered
        if(model.watercon.i.med > model.ref.i.90 | model.watercon.i.med < model.ref.i.10){
          alteration.status[j] <- "likely_altered"
          #if it is altered determine direction of alteration high or low
          if(model.watercon.i.med > model.ref.i.90){
            #direction is high when watercon median is larger than the ref 90th percentile
          ...
          count.in.range <- length(which(model.watercon.ffms.i <= model.ref.i.90 & model.watercon.ffms.i >= model.ref.i.10))
          percent.inrange <- count.in.range/length(na.omit(model.watercon.ffms.i))
      ...
      out.row2 <- c("", sites[i], "", ffm.list[j], alteration.status[j], alteration.direction[j], "", unique(FFM.percentiles.sub.ffm$region), unique(FFM.percentiles.sub.ffm$scenario), "watercon", "")
      #save in output df
...
write.csv(alteration.df.overall, file=paste0(out.dir, "ffm_alteration_watercon_all.csv"), row.names = FALSE)
...
write.csv(alteration.df.overall.join, file=paste0(out.dir, "ffm_alteration_watercon_all.csv"), row.names=FALSE)
...
write.csv(component.alt.df, file=paste0(out.dir, "component_alteration_watercon_all.csv"), row.names=FALSE)
